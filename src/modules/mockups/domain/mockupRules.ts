 ...
 */
// 2026-07-25 수정: 사용자가 "배너"로 요청한 건 SNS 광고 배너가 아니라
// 오프라인 실물 배너(가게 앞 스탠드 배너, 현수막 등)였다 -- 원래 문구가
// 디지털 광고로 잘못 해석되어 정정.
const OFFLINE_BANNER_SCENE =
  "가게 앞에 설치된 오프라인 실물 배너(스탠드 배너, 현수막, 롤업 배너, 윈도우 " +
  "배너, 입간판형 배너 등)가 실제 매장 환경에서 촬영된 사진. 배너에 로고만 " +
  "덩그러니 있으면 안 되고, 실제 배너처럼 짧고 굵은 홍보 문구가 로고와 함께 " +
  "구성된 완성된 배너로 표현한다. 각진 기하학적 레이아웃과 선명한 컬러 대비로 " +
  "지나가는 사람에게도 핵심 메시지가 바로 읽히게 구성하고, 실제 옥외 환경(보도, " +
  "매장 외벽 등)과 자연스럽게 어우러지는 실사 사진으로 표현한다.";

/**
 * 유니폼은 업종에 따라 실제로 다른 의류가 맞다(카페=앞치마, 병원=의료
 * 가운, 건설=작업복 등) -- 2026-07-25 사용자 지시: "의료계인데 로고가
 * 카페 앞치마에 적용되면 안 된다", 예시 갤러리는 다양해도 되지만 실제
 * 렌더링은 반드시 업종에 맞아야 한다. 키워드 매칭으로 결정(추가 AI 호출
 * 없음, MOCKUP_CATEGORY_KEYWORDS와 동일한 방식). 매칭 안 되면 범용 앞치마로
 * 폴백한다.
 */
const UNIFORM_TYPE_BY_INDUSTRY: { keywords: string[]; garment: string }[] = [
  { keywords: ["카페", "커피"], garment: "카페 직원이 착용한 앞치마" },
  { keywords: ["레스토랑", "외식", "식품", "농수산물"], garment: "셰프가 착용한 더블브레스트 셰프 재킷과 앞치마" },
  { keywords: ["병원", "의원", "클리닉"], garment: "의료진이 착용한 흰색 의료 가운(스크럽)" },
  { keywords: ["제조업"], garment: "작업자가 착용한 안전 조끼가 포함된 작업복" },
  { keywords: ["리테일", "쇼핑몰"], garment: "매장 직원이 착용한 폴로셔츠 유니폼" },
  { keywords: ["뷰티", "화장품"], garment: "미용사가 착용한 뷰티 가운" },
  { keywords: ["패션", "의류"], garment: "브랜드 로고가 프린트된 캐주얼 티셔츠" },
  { keywords: ["여행", "숙박"], garment: "호텔 직원이 착용한 재킷형 유니폼" },
  { keywords: ["피트니스", "헬스"], garment: "트레이너가 착용한 트레이닝 유니폼(집업 재킷)" },
  { keywords: ["반려동물"], garment: "펫 그루머가 착용한 그루밍 앞치마" },
];
const DEFAULT_UNIFORM_GARMENT = "직원이 착용한 앞치마 또는 폴로셔츠 유니폼";

function resolveUniformGarment(industry: string | undefined): string {
  if (!industry) return DEFAULT_UNIFORM_GARMENT;
  const match = UNIFORM_TYPE_BY_INDUSTRY.find((entry) => entry.keywords.some((k) => industry.includes(k)));
  return match?.garment ?? DEFAULT_UNIFORM_GARMENT;
}
...
export const MOCKUP_CATEGORY_SCENE_TEMPLATES: Record<MockupCategory, string> = {
  business_card:
    ...
    "브랜드에 맞는 무드로 연출한다.",
  signboard:
    "매장에 설치된 간판이 자연광 아래 촬영된 실사 사진. 전면 채널레터형, 옆면 " +
    "돌출형, 아크릴 라이트박스형, 입간판형 중 매장 환경에 자연스러운 한 형태로 " +
    "표현하고, 큼직하고 읽기 쉬운 타이포그래피와 절제된 색상 팔레트, 넉넉한 " +
    "여백을 갖춘 미니멀한 사이니지 스타일로 주변 건축 요소와 자연스럽게 " +
    "어우러지게 연출한다.",
  mobile_app:
    "실제 스마트폰 기기 프레임 안에 앱 UI 화면이 표시된 장면. 화면에 로고만 " +
    "덩그러니 떠 있으면 안 되고, 실제 운영 중인 앱처럼 보이도록 상단 내비게이션 " +
    "바, 버튼, 아이콘, 샘플 텍스트/카드 목록 등 그럴듯한 UI 요소로 화면을 가득 " +
    "채운 완성된 인터페이스로 표현한다. 카드 기반 레이아웃과 넉넉한 여백, 부드러운 " +
    "그림자로 은은한 입체감(소프트 UI)을 살리고, 손에 들려 있거나 책상 위에 " +
    "자연스럽게 놓인 구도로 촬영한다.",
  website_hero:
    "노트북 화면에 표시된 웹사이트 히어로 섹션이 책상 위 작업 공간에서 촬영된 " +
    "장면. 화면에 로고만 덩그러니 떠 있으면 안 되고, 실제 운영 중인 웹사이트처럼 " +
    "보이도록 헤드라인 카피, 보조 설명 문구, 최소 1개의 시각 요소(사진 또는 " +
    "일러스트), 상단 내비게이션 메뉴, 버튼(CTA)까지 그럴듯한 내용으로 채워진 " +
    "완성된 히어로 섹션으로 표현한다. 명확한 그리드와 큼직한 타이포그래피로 " +
    "임팩트 있는 구도를 만들고, 브랜드 톤에 따라 절제된 팔레트 또는 대담한 컬러 " +
    "중 한 방향으로 일관되게 연출한다.",
  brochure:
    "테이블 위에 놓인 브로슈어 표지가 촬영된 장면. 표지에 로고만 덩그러니 있으면 " +
    "안 되고, 실제 브로슈어처럼 짧은 타이틀 문구와 절제된 그래픽 요소가 함께 " +
    "구성된 완성된 표지로 표현한다. 에디토리얼 감각의 절제된 연출과 여백을 " +
    "살리고, 펜이나 커피컵 같은 소품을 은은하게 곁들여 실제 사용 맥락을 " +
    "자연스럽게 암시한다.",
  poster:
    "벽에 부착된 포스터가 자연광 또는 실내 조명 아래 촬영된 장면. 포스터에 로고만 " +
    "덩그러니 있으면 안 되고, 실제 포스터처럼 굵은 헤드라인 카피와 보조 문구가 " +
    "로고와 함께 구성된 완성된 포스터로 표현한다. 대담한 색 대비와 큼직한 " +
    "타이포그래피가 두드러지는 구도로, 실제 인쇄물처럼 또렷하고 정돈된 느낌을 " +
    "살린다.",
  package:
  ...
  leaflet:
    ...
    "인쇄물처럼 정돈되어 보이게 연출한다.",
  banner: OFFLINE_BANNER_SCENE,
  uniform:
    `${DEFAULT_UNIFORM_GARMENT}이 옷걸이에 걸려 있거나 자연스럽게 착용된 장면. 은은한 ` +
    "자수 또는 프린트 형태로 가슴 부분에 브랜딩이 절제되게 위치하고, 어시 " +
    "뉴트럴 톤(모카·올리브·세이지·차콜) 배경으로 차분하게 연출한다.",
};

/**
 * industry가 주어지면(실제 목업 생성 시 프로젝트 인터뷰에서 가져온 값)
 * uniform 카테고리는 업종에 맞는 의류 종류로 바뀐다 -- 나머지 카테고리는
 * 기존처럼 고정 문구를 그대로 쓴다.
 */
export function buildMockupCategorySceneDirective(category: MockupCategory, industry?: string): string {
  if (category === "uniform") {
    const garment = resolveUniformGarment(industry);
    return (
      `${garment}이 옷걸이에 걸려 있거나 자연스럽게 착용된 장면. 은은한 ` +
      "자수 또는 프린트 형태로 가슴 부분에 브랜딩이 절제되게 위치하고, 어시 " +
      "뉴트럴 톤(모카·올리브·세이지·차콜) 배경으로 차분하게 연출한다."
    );
  }
