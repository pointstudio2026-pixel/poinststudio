...
const DELIVERABLE_OBJECTIVES: Record<string, string> = {
  ...
  포스터:
    ...
    "여백을 적극적으로 활용해 구성한다. 대담한 색 대비(예: 짙은 톤과 선명한 포인트 컬러의 " +
    "조합)로 시선을 사로잡고, 필요하다면 은은한 그레인·텍스처로 따뜻하고 입체적인 " +
    "느낌을 더하되 과하게 산만해지지 않게 절제한다. 실제 인쇄물로 제작 가능한 깔끔한 " +
    "구도를 유지한다.",
  리플렛:
    ...
    "위계를 갖추게 해 실제 인쇄물처럼 또렷하고 정돈된 구도로 구성한다. 브랜드 톤에 " +
    "따라 대담한 고대비 컬러 또는 자연스러운 어시 톤(올리브·클레이·베이지) 중 한 " +
    "방향을 선택하고, 각진 형태보다 부드러운 곡선 요소를 곁들여 친근한 느낌을 더한다.",
  브로슈어:
    "브랜드를 표현하는 브로슈어 표지 디자인을 생성한다. 편집 디자인(에디토리얼) 감각의 정돈된 " +
    "레이아웃과 충분한 여백, 명확한 타이포그래피 위계를 갖춘다. 브랜드 톤에 따라 대담한 " +
    "컬러와 큰 타이포그래피로 임팩트를 주는 방향, 또는 자연스러운 어시 톤과 부드러운 " +
    "곡선 형태로 차분함을 주는 방향 중 하나를 선택해 일관되게 표현한다.",
  명함:
    ...
    "등 나머지 정보를 간결하고 정확한 정렬로 배치한다. 브랜드 톤에 따라 짙은 배경에 " +
    "밝은 잉크로 고급스러움을 강조하는 다크 미니멀 방향, 어시 톤과 기하학적 패턴으로 " +
    "정갈함을 강조하는 방향 중 하나를 선택한다. 실제 인쇄 가능한 평면 디자인으로 " +
    "표현한다.",
  패키지:
