...
const DELIVERABLE_OBJECTIVES: Record<string, string> = {
  ...
  "앱 디자인":
    ...
    "화면 하나로 구성한다. 카드 기반 레이아웃으로 콘텐츠를 구획하고, 미묘한 그림자와 " +
    "부드러운 입체감(소프트 UI)으로 은은한 깊이감을 더하되 화려한 장식은 피한다.",
  웹사이트:
    "브랜드를 표현하는 웹사이트 디자인을 생성한다. 브라우저 창 안에 담긴 데스크톱 랜딩 " +
    "페이지 목업으로 표현하고, 명확한 그리드 위에 화면을 채우는 큼직한 타이포그래피로 " +
    "임팩트 있는 히어로 섹션 하나로 구성한다. 브랜드 톤에 따라 절제된 팔레트와 넉넉한 " +
