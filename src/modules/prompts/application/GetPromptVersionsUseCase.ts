...
import type { PromptVersion } from "@/modules/prompts/domain/Prompt";
...
export class GetPromptVersionsUseCase {
  ...
  async execute(input: { projectId: string; userId: string }): Promise<PromptVersion[]> {
    ...
    return this.promptRepository.listVersions(prompt.id);
  }
}
