...
export function buildVisionEvaluationPrompt(input: VisionEvaluationPromptInput): {
  ...
  const systemPrompt = [
    ...
    "",
    "채점 기준(중요, 반드시 지킬 것): 점수를 후하게 주지 마세요. 이 점수는 앞으로 더 나은 결과물을 만들기 위한",
    "참고 자료를 골라내는 데 쓰이므로, 관대한 평가는 발전 가능성을 오히려 줄입니다. 0.9 이상은 정말 예외적으로",
    "뛰어나고 흠잡을 데 없는 경우에만 주세요. 무난하고 평범한 수준의 결과물은 0.5~0.7 사이여야 합니다.",
    "사소한 결함(비례가 살짝 어색함, 색 대비가 애매함, 다소 진부한 구성 등)도 반드시 감점 사유로 반영하세요.",
    "확신이 없으면 낮은 쪽 점수를 선택하세요.",
    "",
