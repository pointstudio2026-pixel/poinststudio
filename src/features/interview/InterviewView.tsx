...
import { IndustryPicker } from "@/features/interview/IndustryPicker";
        ...
        <section className="flex flex-col gap-3">
          ...
          {currentQuestion.type === "select" ? (
            <div className="flex flex-col gap-2">
              ...
              ) : currentQuestion.key === "industry" ? (
                <IndustryPicker
                  selectedName={answers[currentQuestion.key] ?? ""}
                  onSelect={(name) => handleChange(name)}
                />
              ) : (
                <>
                  <input
                  ...
                  <ul className="flex max-h-64 flex-col gap-1 overflow-y-auto rounded-xl border border-line p-1">
                    ...
                    {(currentQuestion.options ?? []).filter((option) => matchesOptionSearch(option, optionSearch))
                      .length === 0 && <li className="px-3 py-2 text-sm text-muted">{t("interview.noSearchResults")}</li>}
                  </ul>
                </>
              )}
              <ul className="flex flex-col gap-1">
